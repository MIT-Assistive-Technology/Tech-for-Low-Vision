...
var $author$project$Main$defaults = function (flags) {
	...
	return _Utils_Tuple2(
		{fileInput: '', h: height, mode: $author$project$Main$Input, params: $author$project$Main$params, src: 'nothing', typing: false, w: width},
...
var $author$project$Main$Display = {$: 'Display'};
var $author$project$Main$Shortcuts = {$: 'Shortcuts'};
var $author$project$Main$delta = 0.05;
...
var $author$project$Main$imgDecoder = A2($elm$json$Json$Decode$field, 'path', $elm$json$Json$Decode$string);
	...
	function (path, query) {
		var queryParams = A2(
			...
			A2(
				...
				_List_fromArray(
					[
						_Utils_Tuple2('file', path),
						_Utils_Tuple2(
						'poseX',
						$elm$core$String$fromFloat(query.poseX)),
						_Utils_Tuple2(
						'poseY',
						$elm$core$String$fromFloat(query.poseY)),
						_Utils_Tuple2(
						'poseZ',
						$elm$core$String$fromFloat(query.poseZ)),
						_Utils_Tuple2(
						'dirX',
						$elm$core$String$fromFloat(query.dirX)),
						_Utils_Tuple2(
						'dirY',
						$elm$core$String$fromFloat(query.dirY)),
						_Utils_Tuple2(
						'dirZ',
						$elm$core$String$fromFloat(query.dirZ)),
						_Utils_Tuple2(
						'n',
						$elm$core$String$fromInt(query.n)),
						_Utils_Tuple2(
						'i',
						$elm$core$String$fromInt(query.i))
					])));
		var baseUrl = 'http://localhost:5000/get';
		var fullUrl = baseUrl + ('?' + queryParams);
		return $elm$http$Http$get(
			{
				expect: A2($elm$http$Http$expectJson, $author$project$Main$Received, $author$project$Main$imgDecoder),
				url: fullUrl
			});
	});
	...
	function (msg, model) {
		var zUp = function (p) {
			return _Utils_update(
				p,
				{poseZ: p.poseZ + $author$project$Main$delta});
		};
		var zDown = function (p) {
			return _Utils_update(
				p,
				{poseZ: p.poseZ - $author$project$Main$delta});
		};
		var yUp = function (p) {
			return _Utils_update(
				p,
				{poseY: p.poseY + $author$project$Main$delta});
		};
		var yDown = function (p) {
			return _Utils_update(
				p,
				{poseY: p.poseY - $author$project$Main$delta});
		};
		var xUp = function (p) {
			return _Utils_update(
				p,
				{poseX: p.poseX + $author$project$Main$delta});
		};
		var xDown = function (p) {
			return _Utils_update(
				p,
				{poseX: p.poseX - $author$project$Main$delta});
		};
		var _void = $author$project$Main$return(model);
		var prevFrame = function (p) {
			return _Utils_update(
				p,
				{
					i: (p.i > 0) ? (p.i - 1) : p.i
		...
		var nextFrame = function (p) {
			return _Utils_update(
				p,
				{
					i: (_Utils_cmp(p.i, p.n) < 0) ? (p.i + 1) : p.i
				});
		};
		var next = _Utils_Tuple2(
			_Utils_update(
				model,
				{mode: $author$project$Main$Display, typing: false}),
			A2($author$project$Main$getImage, model.fileInput, model.params));
		var moreFrames = function (p) {
			return _Utils_update(
				p,
				{n: p.n + 1});
		};
		var lift = function (f) {
			...
			return _Utils_Tuple2(
				_new,
				A2($author$project$Main$getImage, _new.fileInput, _new.params));
		};
		var lessFrames = function (p) {
			return _Utils_update(
				p,
				{
					n: ((_Utils_cmp(p.n, p.i) > 0) && (p.n > 1)) ? (p.n - 1) : p.n
				});
		};
		var dirZ = function (p) {
			return _Utils_update(
						...
						switch (key) {
							...
							case 'b':
								return $author$project$Main$return(
									_Utils_update(
										model,
										{mode: $author$project$Main$Input}));
							case '?':
								return $author$project$Main$return(
									_Utils_update(
										model,
										{mode: $author$project$Main$Shortcuts}));
							case 'j':
								return lift(nextFrame);
							case 'k':
								return lift(prevFrame);
							case 'h':
								return lift(lessFrames);
							case 'l':
								return lift(moreFrames);
							case 'a':
								return lift(xDown);
							case 'd':
								return lift(xUp);
...
var $author$project$Main$vh = F2(
	function (model, percent) {
		return (model.h * percent) / 100;
	});
var $author$project$Main$vh2pt = F2(
	function (model, ratio) {
		return A2(
			$elm$core$Basics$composeL,
			$elm$core$Basics$round,
			$author$project$Main$vh(model))(ratio);
	});
var $author$project$Main$vh2px = F2(
	function (model, ratio) {
		return $mdgriffith$elm_ui$Element$px(
			A2($author$project$Main$vh2pt, model, ratio));
	});
var $author$project$Main$vw = F2(
	function (model, percent) {
		return (model.w * percent) / 100;
	});
var $author$project$Main$vw2pt = F2(
	function (model, ratio) {
		return A2(
			$elm$core$Basics$composeL,
			$elm$core$Basics$round,
			$author$project$Main$vw(model))(ratio);
	});
var $author$project$Main$vw2px = F2(
	function (model, ratio) {
		return $mdgriffith$elm_ui$Element$px(
			A2($author$project$Main$vw2pt, model, ratio));
	});
var $author$project$Main$white = A3($mdgriffith$elm_ui$Element$rgb255, 255, 255, 255);
var $author$project$Main$displayBox = function (model) {
	var w = $author$project$Main$vw2px(model);
	var p = model.params;
	var h = $author$project$Main$vh2px(model);
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						...
						A2(
							...
							A2($author$project$Main$vw2pt, model, 90),
							h(90)))
					]),
				{description: 'cross section', src: '../' + model.src}),
				A2(
				...
				$mdgriffith$elm_ui$Element$text(
					'Frame ' + ($elm$core$String$fromInt(p.i) + (' of ' + ($elm$core$String$fromInt(p.n) + (', ' + (direction(model) + ' direction'))))))),
				A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
					[
				...
				$mdgriffith$elm_ui$Element$text(
					'Pose (' + (floating(p.poseX) + (', ' + (floating(p.poseY) + (', ' + (floating(p.poseZ) + ')')))))))
			]));
};
var $author$project$Main$FileInput = function (a) {
	return {$: 'FileInput', a: a};
};
var $author$project$Main$ToDisplay = {$: 'ToDisplay'};
...
var $author$project$Main$primary = A3($mdgriffith$elm_ui$Element$rgb255, 25, 128, 230);
...
var $author$project$Main$SetTyping = function (a) {
	return {$: 'SetTyping', a: a};
};
...
var $author$project$Main$typed = _List_fromArray(
	[
		$mdgriffith$elm_ui$Element$Events$onFocus(
		$author$project$Main$SetTyping(true)),
		$mdgriffith$elm_ui$Element$Events$onLoseFocus(
		$author$project$Main$SetTyping(false))
	]);
				...
				_List_fromArray(
					[
						...
						A2(
							$elm$core$List$cons,
							$mdgriffith$elm_ui$Element$spacing(20),
							$author$project$Main$typed),
						{
							label: A2(
								$mdgriffith$elm_ui$Element$Input$labelAbove,
								_List_Nil,
								$mdgriffith$elm_ui$Element$text('Enter file name with extension, like model.glb')),
							onChange: $author$project$Main$FileInput,
						...
						_List_fromArray(
							[
								...
								$mdgriffith$elm_ui$Element$padding(10),
								$mdgriffith$elm_ui$Element$Background$color($author$project$Main$primary),
							...
							]),
						{
							...
							onPress: $elm$core$Maybe$Just($author$project$Main$ToDisplay)
						})
					]))
			]));
};
...
var $mdgriffith$elm_ui$Element$layout = $mdgriffith$elm_ui$Element$layoutWith(
	{options: _List_Nil});
var $author$project$Main$shortcutBox = A2(
	$mdgriffith$elm_ui$Element$column,
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$centerX,
	...
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$text('n: go to model viewer'),
			$mdgriffith$elm_ui$Element$text('b: change model file'),
			$mdgriffith$elm_ui$Element$text('j: next frame'),
			$mdgriffith$elm_ui$Element$text('k: previous frame'),
			$mdgriffith$elm_ui$Element$text('h: reduce frame count by one'),
			$mdgriffith$elm_ui$Element$text('l: increase frame count by one'),
			$mdgriffith$elm_ui$Element$text('d: slightly increase x coordinate'),
			$mdgriffith$elm_ui$Element$text('a: slightly decrease x coordinate'),
			$mdgriffith$elm_ui$Element$text('w: slightly increase y coordinate'),
			$mdgriffith$elm_ui$Element$text('s: slightly decrease y coordinate'),
			$mdgriffith$elm_ui$Element$text('q: slightly increase z coordinate'),
			$mdgriffith$elm_ui$Element$text('e: slightly decrease z coordinate'),
			$mdgriffith$elm_ui$Element$text('x: view cross sections along x axis'),
			$mdgriffith$elm_ui$Element$text('y: view cross sections along y axis'),
			$mdgriffith$elm_ui$Element$text('z: view cross sections along z axis'),
			$mdgriffith$elm_ui$Element$text('?: show this help screen')
		]));
	...
	switch (_v0.$) {
		...
		case 'Display':
			return A2(
				$mdgriffith$elm_ui$Element$layout,
				_List_Nil,
				$author$project$Main$displayBox(model));
		default:
			return A2($mdgriffith$elm_ui$Element$layout, _List_Nil, $author$project$Main$shortcutBox);
	}
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{init: $author$project$Main$defaults, subscriptions: $author$project$Main$subscriptions, update: $author$project$Main$update, view: $author$project$Main$view});
